...
if st.button('Predict'):
    if crop_name != 'Select crop':
       ...
       # Graphical representation
        ...
        # Plotting the required levels
        ax.plot(index, required, marker='o', label='Required', color='#44AFF8')  # Blue color for required levels

        # Plotting the current levels with conditional colors
        for i in index:
            color = '#DAF7A6' if current[i] == required[i] else '#FF5733'  # Green or Red
            ax.plot(i, current[i], marker='o', color=color, label='Current' if i == 0 else "")

        # Connecting the points for current levels
        ax.plot(index, current, color='gray', linestyle='dashed')
